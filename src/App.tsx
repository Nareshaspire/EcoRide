...
import { doc, getDoc, setDoc, collection, query, where, onSnapshot, orderBy, limit, addDoc, updateDoc, arrayUnion, Timestamp } from 'firebase/firestore';
import { auth, db, signInWithGoogle, logout } from './firebase';
import { UserProfile, Ride, Message, Review, RecurringTrip } from './types';
...
const RideSearch = ({ user }: { user: User }) => {
  ...
  const suggestedRides = rides.filter(r => 
    recurringTrips.some(t => 
      ...
      t.role !== (r.driverId === user.uid ? 'driver' : 'rider') // Match opposite role
    )
  ).slice(0, 3);
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          >
            <div className="flex justify-between items-start mb-8">
              <div>
                <span className={cn(
                  ...
                  ride.status === 'completed' ? "bg-blue-100 text-blue-700" : "bg-gray-100 text-gray-600"
                )}>
                  {ride.status}
                </span>
            ...
            {isRider && ride.status !== 'completed' && (
              <div className="mt-8 p-4 bg-emerald-50 text-emerald-700 rounded-2xl font-bold text-center flex items-center justify-center gap-2">
            ...
            {isDriver && ride.status === 'open' && (
              <button 
                ...
                className="w-full bg-gray-900 text-white py-4 rounded-2xl font-bold text-lg hover:bg-emerald-600 transition-all shadow-lg mt-8"
              >
                Mark as Completed
              </button>
            )}
